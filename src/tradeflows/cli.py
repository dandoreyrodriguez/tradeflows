...
def main() -> None:
    ...
    # Comtrade
    # ------------------
    comtrade = sub.add_parser(
        "download_comtrade", help="Download data from UN Comtrade"
    )
    comtrade.add_argument(
        "--iso3", required=True, help="Comma-separated ISO3 codes, e.g. GBR, USA"
    )
    ...
    comtrade.add_argument(
        "--dataset", default="Tariffline", choices=["Tariffline", "Final"]
    )
    comtrade.add_argument("--freq", default="M", choices=["M", "A"])
    comtrade.add_argument("--type", default="C")
    comtrade.add_argument("--cl", default="HS")
    comtrade.add_argument(
        "--decompress", type=str_to_bool, default=True, help="true/false"
    )
    comtrade.add_argument(
        "--overwrite", type=str_to_bool, default=False, help="true/false"
    )
    ...
    census = sub.add_parser(
        "download_census", help="Download HS tariff estimates from US Census API"
    )
    ...
    census.add_argument(
        "--codes",
        default=None,
        help="Comma-separated hs codes. Default is to use the handover parquet created by the comtrade download.",
    )
    # handover selectors (when --codes not supplied)
    census.add_argument(
        "--handover-dataset", default="Tariffline", choices=["Tariffline", "Final"]
    )
